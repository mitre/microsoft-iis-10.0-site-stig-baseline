...
```yaml
  ...
  # used by the Microsoft IIS 10.0 Site STIG profile.
...
```bash
# Using `ssh` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/microsoft-iis-10.0-site-stig-baseline/archive/main.tar.gz --input-file=<your_inputs_file.yml> -t ssh://<hostname>:<port> --sudo --reporter=cli json:<your_results_file.json>

# Using `winrm` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/microsoft-iis-10.0-site-stig-baseline/archive/master.tar.gz --target winrm://<hostip> --user '<admin-account>' --password=<password> --input-file=<path_to_your_inputs_file/name_of_your_inputs_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/microsoft-iis-10.0-site-stig-baseline.git
bundle exec [inspec or cinc-auditor] archive microsoft-iis-10.0-site-stig-baseline
...
```bash
cd microsoft-iis-10.0-site-stig-baseline
git pull
cd ..
bundle exec [inspec or cinc-auditor] archive microsoft-iis-10.0-site-stig-baseline --overwrite
