...
This InSpec Profile was created to facilitate testing and auditing of `Microsoft IIS 10.0 Site`
...
The Microsoft IIS 10.0 Site STIG Profile uses the [InSpec](https://github.com/inspec/inspec)
...
Requirements associated with the Microsoft IIS 10.0 Site STIG are derived from the
...
The Microsoft IIS 10.0 Site STIG profile checks were developed to provide technical implementation
...
```
# Using `ssh` transport
bundle exec inspec exec https://github.com/mitre/Microsoft IIS 10.0 Site Security Technical Implementation Guide/archive/main.tar.gz --input-file=<your_inputs_file.yml> -t ssh://<hostname>:<port> --sudo --reporter=cli json:<your_results_file.json>

# Using `winrm` transport
bundle exec inspec exec https://github.com/mitre/Microsoft IIS 10.0 Site Security Technical Implementation Guide/archive/master.tar.gz --target winrm://<hostip> --user '<admin-account>' --password=<password> --input-file=<path_to_your_inputs_file/name_of_your_inputs_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/Microsoft IIS 10.0 Site Security Technical Implementation Guide.git
bundle exec inspec archive Microsoft IIS 10.0 Site Security Technical Implementation Guide
...
```
cd Microsoft IIS 10.0 Site Security Technical Implementation Guide
git pull
cd ..
bundle exec inspec archive Microsoft IIS 10.0 Site Security Technical Implementation Guide --overwrite
